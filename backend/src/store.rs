...
impl Store {
    ...
    pub fn list_sent(&self, limit: u32) -> Result<Vec<Draft>> {
        let conn = self.lock()?;
        let mut stmt = conn.prepare(
            "SELECT * FROM drafts WHERE state = 'sent' ORDER BY sent_at DESC LIMIT ?1",
        )?;
    ...
    pub fn get(&self, id: &str) -> Result<Option<Draft>> {
        let conn = self.lock()?;
        Ok(conn
            .query_row("SELECT * FROM drafts WHERE id = ?1", params![id], row_to_draft)
            .optional()?)
    }
...
mod tests {
    ...
    fn only_one_claim_wins() {
        ...
        s.set_trigger("d1", Trigger::Concurrency { below: 9 }).unwrap();
    ...
    fn all_done_waits_for_the_node_to_have_been_busy() {
        ...
        let quiet = Readings { running: Some(0), usage: vec![], now_ms: 0 };
        assert!(
        ...
        let busy = Readings { running: Some(2), usage: vec![], now_ms: 0 };
    ...
    fn re_arming_resets_the_busy_latch() {
        ...
        s.ready(&Readings { running: Some(1), usage: vec![], now_ms: 0 }).unwrap();
