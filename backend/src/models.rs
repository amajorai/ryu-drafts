...
// `ToSchema` so the OpenAPI document describes the condition an agent has to supply
// when it arms a draft. Without it `arm` reaches the model as a tool with an
// unfillable argument.
...
impl Trigger {
    ...
    pub fn is_satisfied(&self, r: &Readings, seen_busy: bool) -> bool {
        match self {
            ...
            Trigger::Concurrency { below } => r.running.is_some_and(|running| running < *below),
...
impl From<Draft> for DraftRow {
    fn from(draft: Draft) -> DraftRow {
        ...
        let waiting_for = match draft.state {
            ...
            State::Armed if draft.trigger == Trigger::AllDone && !draft.seen_busy => {
