...
Drafts for Ryu — a durable outbox for unsent messages: the draft store, the trigger vocabulary that decides when a draft becomes sendable, and the claim protocol that keeps two desktop windows from sending the same draft twice, all held out-of-process in a SQLite-backed sidecar Core proxies to on loopback.
...
- Binary: `ryu-drafts` from the [Ryu releases](https://github.com/amajorai/ryu/releases).
- Crate: `cargo install ryu-drafts`.
...
A durable outbox for messages you have not sent yet.

Two things happen today to text you type and don't send: it dies with the tab, or it
sits in a chat you have to remember to go back to. Drafts fixes both. Unsent composer
text is kept in one node-wide store — not inside the conversation it happened to be
typed in — so it survives closing the chat, restarting the app, and moving to another
project. And a draft can be *armed*: give it a condition and it sends itself when the
condition holds.
